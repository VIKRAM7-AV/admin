...
const EditFaq = (props: Props) => {
  ...
  useEffect(() => {
    if (data?.layout?.faq) {
      if (data.layout.faq.length === 0) {
        // Add a default FAQ if there are no existing ones
        setQuestions([
          {
            _id: Date.now().toString(), // Generate a temporary unique ID
            question: "What is your refund policy?",
            answer: "We offer a 30-day refund policy for unused services.",
            active: false,
          },
        ]);
      } else {
  ...
  const handleEdit = async () => {
    if (
      data?.layout?.faq && // Check if data.layout.faq exists
  ...
  return (
    <>
      ...
      ${data?.layout?.faq && // Check for null before accessing faq
                  (areQuestionsUnchanged(data.layout.faq, questions) ||
                    isAnyQuestionEmpty(questions))
                  ? "!cursor-not-allowed"
